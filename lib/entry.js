...
/**
 * Where device state lives — deliberately *outside* `$DSH_HOME`.
 *
 * `$DSH_HOME` is the tree the desktop shell watches for configuration changes.
 * Writing state there is how this code once closed a feedback loop (write →
 * watcher fires → profile patch re-applied → plugin re-mounted → more writes)
 * that ended in a 4 GB heap and an aborted harness. Device state is not
 * configuration and has no business anywhere near that watcher, so it goes to
 * the user's local app-data directory instead. `DSH_LAN_HOP_STATE` overrides it.
 */
function stateFile() {
  const explicit = process.env.DSH_LAN_HOP_STATE
  if (typeof explicit === 'string' && explicit.trim() !== '') return explicit.trim()
  const base =
    process.env.LOCALAPPDATA ?? process.env.XDG_STATE_HOME ?? path.join(os.homedir(), '.local', 'state')
  return path.join(base, 'dsh-lan-hop', 'devices.json')
}
...
/**
 * Device state, cached in memory and flushed at most once every 30 seconds.
 *
 * This matters far more than it looks. The file lives under `$DSH_HOME`, which
 * is exactly the tree the desktop shell watches for configuration changes, so a
 * write on every authenticated request closes a feedback loop — request → write
 * → watcher fires → profile patch re-applied → plugin re-mounted → more requests
 * — and that is how this plugin once killed the harness: roughly 4 GB of heap in
 * under a minute, then `exit code 134`. Reads and writes per request are now an
 * in-memory document with a debounced flush.
 */
const stateCache = { document: undefined, loadedAt: 0, dirty: false, timer: undefined }
...
function readState() {
  const now = Date.now()
  if (stateCache.document !== undefined && now - stateCache.loadedAt < 5_000) return stateCache.document
  let document
  try {
    const parsed = JSON.parse(fs.readFileSync(stateFile(), 'utf8'))
    if (typeof parsed?.signingKey === 'string' && typeof parsed?.devices === 'object') document = parsed
  } catch {
    /* fall through to a fresh state */
  }
  stateCache.document = document ?? freshState()
  stateCache.loadedAt = now
  return stateCache.document
}

function writeState(document) {
  stateCache.document = document
  stateCache.loadedAt = Date.now()
  stateCache.dirty = true
  if (stateCache.timer !== undefined) return
  stateCache.timer = setTimeout(flushState, STATE_FLUSH_MS)
  // Never hold the process open only to flush this file.
  stateCache.timer.unref?.()
}

/** Write the debounced document out once, if it changed. */
function flushState() {
  stateCache.timer = undefined
  if (stateCache.dirty !== true || stateCache.document === undefined) return
  const file = stateFile()
  try {
    fs.mkdirSync(path.dirname(file), { recursive: true })
    fs.writeFileSync(file, `${JSON.stringify(stateCache.document, undefined, 2)}\n`, 'utf8')
    stateCache.dirty = false
  } catch (error) {
    console.error(`[dsh-mobile-direct] could not persist device state: ${String(error?.message ?? error)}`)
  }
}
 ...
 */
/** Stop watching for a runaway heap; returned by {@link watchHeap}. */
const HEAP_LIMIT_BYTES = 512 * 1024 * 1024

/**
 * Stop the listener if this process's heap runs away.
 *
 * A plugin inside the harness must never be able to take the harness down with
 * it. This entry was once the cause of an out-of-memory abort, so it now watches
 * its own process: if the heap grows past a wide margin above its baseline, the
 * listener closes itself and says so, loud and once.
 */
function watchHeap(stop) {
  let baseline = process.memoryUsage().heapUsed
  let armed = true
  const timer = setInterval(() => {
    const used = process.memoryUsage().heapUsed
    if (used < baseline) baseline = used
    const grown = used - baseline
    if (!armed || grown < HEAP_LIMIT_BYTES) return
    armed = false
    console.error(
      `[dsh-mobile-direct] heap grew ${String(Math.round(grown / 1048576))}MB above baseline — stopping the LAN entry to protect the harness`,
    )
    stop()
  }, 10_000)
  timer.unref?.()
...
/**
 * One entry per port, process-wide.
 *
 * A profile patch can be re-applied (hot reload, desktop reconciliation) without
 * the previous instance being disposed first. Without this guard every
 * re-application would build another listener and another set of closures.
 */
const liveEntries = new Map()

export function createEntry({ ctx, config, log }) {
  const port = Number(config.entryPort ?? 3444)
  const existing = liveEntries.get(port)
  if (existing !== undefined && existing.isRunning()) {
    console.log(`[dsh-mobile-direct] reusing the LAN entry already listening on :${String(port)}`)
    return existing
  }
...
<button type="submit">撤销</button></form></li>`,
  ...
  const instance = {
    ...
    start() {
      if (listening && server !== undefined) return Promise.resolve(server.address())
      return new Promise((resolve, reject) => {
        ...
        server.listen(port, config.entryBind ?? '0.0.0.0', () => {
          server.off('error', reject)
          listening = true
          liveEntries.set(port, instance)
          stopWatching = watchHeap(() => void instance.stop())
          console.log(
            `[dsh-mobile-direct] LAN entry listening on :${String(port)} (harness upstream 127.0.0.1:${String(upstreamPort())})`,
          )
          resolve(server.address())
        })
      })
    },
    /** Stop listening, drop pending connections, and flush device state once. */
    stop() {
      stopWatching?.()
      stopWatching = undefined
      listening = false
      liveEntries.delete(port)
      flushState()
      return new Promise((resolve) => {
        if (server === undefined) return resolve()
        console.log(`[dsh-mobile-direct] LAN entry on :${String(port)} stopped`)
        server.close(() => resolve())
        server.closeAllConnections?.()
      })
    },
    /** Whether this entry is currently holding the port. */
    isRunning() {
      return listening
    },
