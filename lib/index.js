...
const DEFAULTS = {
  // 'direct' points the QR at this plugin's own LAN entry (pairing + proxy on
  // entryPort). 'relay' keeps the original behaviour of asking dsh-relay for a
  // code — useful only where that relay is known to work.
  entryMode: 'direct',
  entryEnabled: true,
  entryPort: 3444,
  entryBind: '0.0.0.0',
...
/**
 * State built from this plugin's own LAN entry — no relay involved.
 *
 * The QR carries the same payload shape the DSH Mobile app already parses, but
 * the address it names is the entry's own listener: the one component here that
 * both pairs the device *and* can mint the harness session the `/api` calls
 * need. That combination is what dsh-relay's proxy is missing.
 */
function entryState(config, entry, candidates, address) {
  const host = address === undefined ? undefined : address.address
  const info = entry.describe(host)
  const payload = {
    v: PAYLOAD_VERSION,
    kind: PAYLOAD_KIND,
    url: info.origin,
  ...
  return {
    ...
    code: info.code,
    expiresAt: info.expiresAt,
    ttlSeconds: Math.max(0, Math.round((info.expiresAt - Date.now()) / 1000)),
    origin: info.origin,
    payload,
    /** What the app scans to enrol and receive a device token. */
    appPayloadText: JSON.stringify(payload),
    /** What a browser opens to enter with a single tap (no typing, no password). */
    browserEntryUrl: info.browserEntryUrl,
    browserUrl: `${info.origin}/`,
    devicesUrl: info.devicesUrl,
  }
}

/** The whole state the UI needs, in one object. */
async function buildState(config, entry, { refresh = false } = {}) {
  const candidates = lanCandidates()
  const address = chooseAddress(config)
  if (config.entryMode === 'direct' && entry !== undefined) {
    if (refresh) entry.describe(address === undefined ? undefined : address.address)
    return entryState(config, entry, candidates, address)
  }
  let relay
  try {
...
function badgeMarkup() {
  ...
  const script = [
    ...
    '    if(target==="app"){noteEl.textContent="用 DSH Mobile App 的「中继 → 配对中继」扫这个码：手机自动配对，拿到设备令牌（默认 30 天，之后不用再扫）。"}',
    '    else{noteEl.textContent="手机浏览器扫这个码直接进入：不用输密码、不用点确认。"}',
...
function handle(request, response, config, entry) {
  ...
  const run = async () => {
    ...
    if (route === '/state.json') {
      ...
      let state
      try {
        state = await buildState(config, entry, { refresh })
      } catch (error) {
    ...
    if (route === '/qr.svg') {
      ...
      const state = await buildState(config, entry, { refresh: url.searchParams.get('refresh') === '1' })
...
/** Mount the routes, the LAN entry, and the badge; take them back down with this plugin. */
export function apply(ctx, config) {
  const resolved = { ...DEFAULTS, ...(config ?? {}) }
  ctx.effect(() => {
    let entry
    if (resolved.entryEnabled) {
      entry = createEntry({ ctx, config: resolved, log: ctx.logger })
      entry
        .start()
        .then((address) => {
          const where = address === null || typeof address !== 'object' ? '0.0.0.0' : address.address
          ctx.logger?.info?.(
            `dsh-mobile-direct: LAN entry listening on http://${String(where)}:${String(resolved.entryPort)} (harness on 127.0.0.1:${String(ctx.webServer.port)})`,
          )
        })
        .catch((error) => {
          // A busy port must not take the plugin down: fall back to relay mode,
          // where the badge still renders and the reason is visible in the panel.
          entry = undefined
          ctx.logger?.warn?.(
            `dsh-mobile-direct: LAN entry could not start (${String(error?.message ?? error)}); falling back to relay mode`,
          )
        })
    }
    const disposers = [
      ctx.webServer.register({
        kind: 'prefix',
        path: PREFIX,
        handler: (request, response) => handle(request, response, resolved, entry),
      }),
    ]
